...
Pointless is a lightweight, open-source collaborative tool for agile planning poker and real-time voting. Designed for simplicity and speed, it allows teams to estimate tasks together in real-time using a clean, intuitive web interface.
...
The project includes a root-level `Dockerfile` that packages both the backend and frontend into a single image, served via Nginx.
...
*Note: In development mode, you may need to update the `API_URL` in `pointless-ui/public/config.json` to point to your local backend.*
